...
resource "aws_security_group" "internal_alb_sg" {
  ...
  description = "Allow HTTP traffic only from inside VPC"
  vpc_id      = var.vpc_id   # FIXED HERE ✅
...
resource "aws_security_group" "public_web_sg" {
  ...
  ingress {
    ...
    to_port     = 22
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"] # SSH from anywhere (not secure, restrict later!)
  }
...
resource "aws_security_group" "private_web_sg" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["10.0.0.0/16"] # Allow only internal traffic
  }
